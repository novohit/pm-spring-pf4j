...
public final class PmSpringPlugin extends Plugin {
    ...
    public void start() {
        ...
        AnnotationConfigApplicationContext customized = businessPlugin.beforeApplicationContextRefresh(baseContext);
        ...
        try {
            ...
            lifecycleEngine.executePhase(PluginLifecyclePhase.BEFORE_CONTEXT_REFRESH, applicationContext);
            applicationContext.refresh();
            businessPlugin.afterApplicationContextReady(applicationContext);
            lifecycleEngine.executePhase(PluginLifecyclePhase.AFTER_CONTEXT_REFRESH, applicationContext);
            everStarted = true;
    ...
    private AnnotationConfigApplicationContext preCreateApplicationContext() {
        ...
        if (!basePackage.equals(pluginId)) {
            throw new PluginRuntimeException(
                    "Plugin ID must match the business plugin package: id=" + pluginId + ", package=" + basePackage);
        }
