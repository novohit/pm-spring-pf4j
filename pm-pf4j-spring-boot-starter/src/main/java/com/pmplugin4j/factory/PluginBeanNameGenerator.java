 ...
 * If @Service("myService") is used, the final name will be "user.myService" - Degrade to the Spring default naming
 * strategy when the plugin ID is "plugin" or empty
 */
public class PluginBeanNameGenerator extends AnnotationBeanNameGenerator {

    private static final Logger log = LoggerFactory.getLogger(PluginBeanNameGenerator.class);
    ...
    public PluginBeanNameGenerator(String pluginId) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Initialized PluginBeanNameGenerator with pluginId='{}', effective prefix='{}'", pluginId,
                    this.pluginPrefix);
        }
    }
    ...
    protected String buildDefaultBeanName(@NonNull BeanDefinition definition) {
        ...
        if (beanClassName == null) {
            ...
            log.warn("BeanDefinition has null beanClassName. Using fallback name: '{}'. Definition: {}", fallbackName,
                    definition);
        ...
        if (StringUtils.hasText(pluginId) && shortClassName.regionMatches(true, 0, pluginId, 0, pluginId.length())) {
        ...
        if (log.isDebugEnabled()) {
            if (!optimizedBaseName.equals(baseName)) {
                log.debug("Optimized bean name from '{}' to '{}' for class '{}'", pluginPrefix + baseName, finalName,
                        beanClassName);
            } else {
    ...
    @Override
    protected String determineBeanNameFromAnnotation(@NonNull AnnotatedBeanDefinition annotatedDefinition) {
        String originalName = super.determineBeanNameFromAnnotation(annotatedDefinition);
        if (originalName == null) {
            if (log.isTraceEnabled()) {
                String className = annotatedDefinition.getBeanClassName();
        ...
        if (log.isDebugEnabled()) {
            String className = annotatedDefinition.getBeanClassName();
            log.debug("Applied plugin prefix to explicit bean name: '{}' → '{}' (class: '{}')", originalName,
                    prefixedName, className != null ? className : "unknown");
        }
        return prefixedName;
    }
}
