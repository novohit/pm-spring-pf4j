...
/**
 * Read-only registry of plugin anonymous endpoints. Injected by the host application into its Spring Security
 * configuration to query anonymous paths and for operational visibility.
 *
 * <p>
 * <strong>Host application integration (MVC):</strong> <pre>{@code
 * &#64;Configuration
 * &#64;EnableWebSecurity
...
public interface PluginAnonymousPathRegistry {
    ...
    /**
     * Check whether a request path + HTTP method combination matches any registered anonymous endpoint. Called on the
