 ...
 * <p>
 * Uses a two-level index: outer key = HTTP method ({@code GET}, {@code POST}, {@code *}, etc.), inner key = path
 * pattern. On {@link #isAnonymous} calls, first checks the exact HTTP method bucket, then the wildcard ({@code *})
 * bucket. Within each bucket, {@link AntPathMatcher} is used for pattern matching so that {@code {variable}} and
 * Ant-style wildcards work correctly.
 */
public class DefaultPluginAnonymousPathRegistry implements PluginAnonymousPathRegistry, PluginAnonymousPathRegistrar {
     ...
     */
    private final ConcurrentHashMap<String, ConcurrentHashMap<String, AnonymousPathEntry>> methodIndex = new ConcurrentHashMap<>();
    ...
    public void register(String pluginId, AnonymousPathEntry entry) {
        ...
        if (previous != null) {
            log.warn("Anonymous path conflict: {}:{} was already registered by plugin '{}', "
                    + "overwritten by plugin '{}'", method, entry.pathPattern(), previous.pluginId(), pluginId);
        }
        ...
        log.info("[Plugin: {}] Registered anonymous endpoint: {} {} -> {}.{}() (reason: {})", pluginId, method,
                entry.pathPattern(), entry.controllerClass(), entry.methodName(), entry.reason());
    }
    ...
    public Collection<AnonymousPathEntry> listAll() {
        return methodIndex.values().stream().flatMap(bucket -> bucket.values().stream()).toList();
    }
    ...
    @Override
    public int getCount() {
        return methodIndex.values().stream().mapToInt(ConcurrentHashMap::size).sum();
    }
