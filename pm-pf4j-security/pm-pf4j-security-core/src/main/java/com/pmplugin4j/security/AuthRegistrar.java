...
public class AuthRegistrar implements PluginResourceRegistrar {
    ...
    public void onAfterContextRefresh(AnnotationConfigApplicationContext pluginCtx) {
        ...
        PluginRouteRegistry routeRegistry = hostCtx.getBeanProvider(PluginRouteRegistry.class).getIfAvailable();
        if (routeRegistry == null) {
            log.debug("[Plugin: {}] No route registry — non-web app, skipping", pluginId);
            return;
        }
        Map<String, Set<String>> allPaths = routeRegistry.getPluginPaths(pluginId);
        Map<String, Set<String>> authenticatedPaths = routeRegistry.getPluginAuthenticatedPaths(pluginId);
