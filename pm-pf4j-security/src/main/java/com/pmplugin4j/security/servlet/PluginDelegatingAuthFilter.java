 ...
 * <p>
 * Delegates all provider-chain execution to {@link AuthChainExecutor}, keeping only platform-specific concerns
 * (SecurityContext, error response).
 */
public class PluginDelegatingAuthFilter extends OncePerRequestFilter {
    ...
    public PluginDelegatingAuthFilter(PmPluginAuthRegistry registry, PluginAnonymousPathRegistry anonymousPaths,
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (existingAuth != null && existingAuth.isAuthenticated()) {
            if (authenticatedPaths.isPluginAuthenticated(wrappedRequest.getMethod(), path)) {
        ...
        // List<Object> return type is for pm-pf4j-core / pm-pf4j-security module isolation.
        ...
        AuthChainExecutor.AuthChainResult result = AuthChainExecutor.execute(providers, wrappedRequest, strategy,
                pluginId);
