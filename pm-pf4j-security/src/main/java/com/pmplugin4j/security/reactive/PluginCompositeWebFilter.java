...
import com.pmplugin4j.security.PluginFilterErrorEvent;
...
/**
 * Composite WebFlux web filter — pre-registered in the host's {@code SecurityWebFilterChain} at a specific position.
 * Delegates to dynamically-registered plugin web filters (sandboxed).
 */
public class PluginCompositeWebFilter implements WebFilter, Ordered {
    ...
    public PluginCompositeWebFilter(PmPluginFilterRegistry registry, PmPluginFilterPosition position, int baseOrder) {
    ...
    public Mono<Void> filter(@NonNull ServerWebExchange exchange, @NonNull WebFilterChain chain) {
        ...
        for (int i = filters.size() - 1; i >= 0; i--) {
            ...
            wrapped = ex -> {
                ...
                return f.filter(ex, trackingNext).onErrorResume(e -> {
                    log.error("[Filter:{}] Plugin web filter error — skipping: {}", position,
                            f.getClass().getSimpleName(), e);
    ...
    private void publishFilterError(WebFilter f, Throwable e) {
        ...
        try {
            ...
            eventPublisher
                .publishEvent(new PluginFilterErrorEvent(this, pluginId, f.getClass().getName(), position, exception));
