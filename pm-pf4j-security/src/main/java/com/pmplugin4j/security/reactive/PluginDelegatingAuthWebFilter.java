 ...
 * <p>
 * Delegates all provider-chain execution to {@link AuthChainExecutor}, keeping only platform-specific concerns (body
 * caching, SecurityContext, error response).
 */
public class PluginDelegatingAuthWebFilter implements WebFilter, Ordered {
    ...
    private final PmPluginAuthRegistry registry;
    ...
    public PluginDelegatingAuthWebFilter(PmPluginAuthRegistry registry, PluginAnonymousPathRegistry anonymousPaths,
            PluginAuthenticatedPathRegistry authenticatedPaths, AuthenticationStrategy strategy,
    ...
    public Mono<Void> filter(@NonNull ServerWebExchange exchange, @NonNull WebFilterChain chain) {
        ...
        return ReactiveSecurityContextHolder.getContext()
            ...
            .switchIfEmpty(Mono.defer(() -> {
                ...
                List<IPluginAuthenticationProvider> providers = (List<IPluginAuthenticationProvider>) (List<?>) providerObjs;
                ...
                return DataBufferUtils.join(exchange.getRequest().getBody()).map(buf -> {
                    byte[] bytes = new byte[buf.readableByteCount()];
                    buf.read(bytes);
                    DataBufferUtils.release(buf);
                    return bytes;
                }).defaultIfEmpty(new byte[0]).flatMap(bodyBytes -> {
                    WebFluxHttpServletRequestAdapter adapter = new WebFluxHttpServletRequestAdapter(exchange,
                            bodyBytes);

                    AuthChainExecutor.AuthChainResult result = AuthChainExecutor.execute(providers, adapter, strategy,
                            pluginId);
    ...
    private Mono<Void> handleResult(ServerWebExchange exchange, WebFilterChain chain,
            AuthChainExecutor.AuthChainResult result, byte[] bodyBytes) {
        if (result instanceof AuthChainExecutor.AuthChainResult.Success success) {
            ...
            challenge.exception().getHeaders().forEach((k, v) -> exchange.getResponse().getHeaders().add(k, v));
            exchange.getResponse().setStatusCode(HttpStatus.valueOf(challenge.exception().getStatusCode()));
            publishEvent(new PluginAuthenticationFailureEvent(this, challenge.pluginId(), challenge.providerName(),
    ...
    private static ServerWebExchange decorateWithBody(ServerWebExchange exchange, byte[] bodyBytes) {
        return exchange.mutate().request(new ServerHttpRequestDecorator(exchange.getRequest()) {
            @Override
            @NonNull
    ...
    private void publishSuccess(AuthChainExecutor.AuthChainResult.Success s) {
        if (eventPublisher == null)
            return;
        try {
            for (AuthenticationStrategy.ProviderAttempt a : s.attempts()) {
                if (a.result() != null && a.result().isAuthenticated()) {
                    publishEvent(new PluginAuthenticationSuccessEvent(this, s.pluginId(),
                            a.provider().getClass().getSimpleName(), a.durationMs(), a.result(),
                            a.provider().getOrder()));
                    return;
                }
            }
    ...
    private void publishChainFailure(AuthChainExecutor.AuthChainResult.Failure f) {
        ...
        try {
            for (int i = f.attempts().size() - 1; i >= 0; i--) {
                AuthenticationStrategy.ProviderAttempt a = f.attempts().get(i);
                if (a.exception() != null) {
                    publishEvent(new PluginAuthenticationFailureEvent(this, f.pluginId(),
                            a.provider().getClass().getSimpleName(), a.durationMs(), a.exception(),
                            a.provider().getOrder()));
                    return;
                }
            }
