 ...
 * <p>
 * Thrown by {@code authenticate()} when the caller must complete an out-of-band challenge before authentication can
 * proceed. The filter responds by setting the given status code + headers rather than 401.
 * <p>
 * {@link #fillInStackTrace()} is overridden to skip stack trace generation — this is a control-flow signal, not a
 * crash.
 * <p>
 * Common uses:
 * <ul>
