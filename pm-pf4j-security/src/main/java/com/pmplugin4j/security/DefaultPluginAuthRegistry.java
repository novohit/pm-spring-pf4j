...
/**
 * Default implementation of {@link PmPluginAuthRegistry}.
 * <p>
 * Data structure: <pre>
 ...
 * </pre>
 * <p>
 * Route lookup uses longest-prefix {@code AntPathMatcher} match to resolve ambiguous patterns (e.g. {@code /api/**} vs
 * {@code /api/v3/**}).
 */
class DefaultPluginAuthRegistry implements PmPluginAuthRegistry {

    private static final Logger log = LoggerFactory.getLogger(DefaultPluginAuthRegistry.class);

    private final ConcurrentHashMap<String, List<IPluginAuthenticationProvider>> providers = new ConcurrentHashMap<>();
    /** pluginId → (HTTP method → Set<pattern>) */
    private final ConcurrentHashMap<String, Map<String, Set<String>>> routeTable = new ConcurrentHashMap<>();
    /** Hot-path flat index: method → (pattern → pluginId) */
    private final ConcurrentHashMap<String, CopyOnWriteArrayList<RouteEntry>> flatIndex = new ConcurrentHashMap<>();
    ...
    // ──── Registration — downcast from Object (interface lives in pm-pf4j-core) ──
    ...
    public void registerProvider(String pluginId, Object provider) {
        ...
        if (!(provider instanceof IPluginAuthenticationProvider p)) {
            throw new IllegalArgumentException(
                    "Expected IPluginAuthenticationProvider, got: " + provider.getClass().getName());
        }
        providers.compute(pluginId, (k, list) -> {
            ...
            for (IPluginAuthenticationProvider existing : list) {
                if (existing.getClass().equals(p.getClass())) {
                    throw new IllegalArgumentException(
                            "Plugin [" + pluginId + "] already has a provider of type " + p.getClass().getName());
                }
            }
            list.add(p);
        ...
        log.info("[AuthRegistry] Registered auth provider #{} for plugin '{}': {} (order={})",
                providers.get(pluginId).size(), pluginId, p.getClass().getSimpleName(), p.getOrder());
    }
    ...
    public void registerRoutes(String pluginId, Map<String, Set<String>> methodPatterns) {
        ...
        routeTable.compute(pluginId, (k, existing) -> {
            if (existing == null) {
                ...
                methodPatterns.forEach((method, patterns) -> m.put(method, Set.copyOf(patterns)));
                return m;
            }
            methodPatterns.forEach((method, patterns) -> {
                existing.merge(method, Set.copyOf(patterns), (oldSet, newSet) -> {
        ...
        log.info("[AuthRegistry] Registered {} route(s) for plugin '{}'", totalPatterns, pluginId);

        // Update flat index (hot-path acceleration)
        methodPatterns.forEach((method, patterns) -> {
            CopyOnWriteArrayList<RouteEntry> list = flatIndex.computeIfAbsent(method,
    ...
    public String lookupPluginId(String httpMethod, String requestUri) {
        ...
        return lookupByMethod(method, requestUri, false)
            .orElseGet(() -> lookupByMethod("*", requestUri, true).orElse(null));
    }

    private Optional<String> lookupByMethod(String method, String requestUri, boolean wildcard) {
        CopyOnWriteArrayList<RouteEntry> entries = flatIndex.get(method);
    ...
    public Collection<ProviderInfo> listProviders() {
        return providers.entrySet().stream().map(e -> {
        ...
        }).sorted(Comparator.comparing(ProviderInfo::pluginId)).toList();
    }
}