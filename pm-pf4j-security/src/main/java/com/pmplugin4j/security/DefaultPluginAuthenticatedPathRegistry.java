 ...
 * <p>
 * Data structure: pluginId → HTTP method → list of path patterns. Read-heavy, write-at-startup — uses
 * {@code CopyOnWriteArrayList} for lock-free reads on the request hot path.
 */
public class DefaultPluginAuthenticatedPathRegistry implements PluginAuthenticatedPathRegistry {

    private final ConcurrentHashMap<String, ConcurrentHashMap<String, CopyOnWriteArrayList<String>>> registry = new ConcurrentHashMap<>();
    /** Hot-path flat index: method → (pattern, pluginId) */
    private final ConcurrentHashMap<String, CopyOnWriteArrayList<PathEntry>> flatIndex = new ConcurrentHashMap<>();
    ...
    public void register(String pluginId, String method, String pattern) {
        ...
        registry.computeIfAbsent(pluginId, k -> new ConcurrentHashMap<>())
            ...
            .add(pattern);
        flatIndex.computeIfAbsent(m, k -> new CopyOnWriteArrayList<>()).add(new PathEntry(pattern, pluginId));
    }

    @Override
    public void unregister(String pluginId) {
        ConcurrentHashMap<String, CopyOnWriteArrayList<String>> removed = registry.remove(pluginId);
        if (removed != null) {
