...
public class PmPluginWebFluxDefaultRouterFunctionRegistry
        implements RouterFunction<ServerResponse>, PmPluginWebFluxRouterFunctionRegistry {
    ...
    private final Map<String, Map<String, Set<String>>> routerFunctionAuthPaths = new ConcurrentHashMap<>();

    public PmPluginWebFluxDefaultRouterFunctionRegistry() {
    ...
    public Mono<HandlerFunction<ServerResponse>> route(@NonNull ServerRequest request) {
        PmPluginWebFluxSecureServerRequest secureServerRequest = new PmPluginWebFluxSecureServerRequest(request);
        return Flux.fromIterable(this.routerFunctions)
            .concatMap(routerFunction -> routerFunction.route(secureServerRequest)
                .map(rf -> (HandlerFunction<ServerResponse>) serverRequest -> {
                    try {
    ...
    @Override
    public void register(String pluginId, Collection<RouterFunction<ServerResponse>> routerFunctions) {
        ...
        for (RouterFunction<ServerResponse> rf : routerFunctions) {
            if (rf instanceof PmRouterFunctions.AnnotatedRouterFunction arf) {
                for (AnonymousRouteDeclaration decl : arf.getDeclarations()) {
                    routerFunctionPaths.computeIfAbsent(pluginId, k -> new ConcurrentHashMap<>())
                        .computeIfAbsent(decl.httpMethod().toUpperCase(), k -> ConcurrentHashMap.newKeySet())
                        .add(decl.pathPattern());
                }
                for (PluginAuthenticatedRouteDeclaration decl : arf.getPluginAuthenticatedDeclarations()) {
                    routerFunctionAuthPaths.computeIfAbsent(pluginId, k -> new ConcurrentHashMap<>())
                        .computeIfAbsent(decl.httpMethod().toUpperCase(), k -> ConcurrentHashMap.newKeySet())
                        .add(decl.pathPattern());
                }
            }
    ...
    @Override
    public void unregister(String pluginId, Collection<RouterFunction<ServerResponse>> routerFunctions) {
