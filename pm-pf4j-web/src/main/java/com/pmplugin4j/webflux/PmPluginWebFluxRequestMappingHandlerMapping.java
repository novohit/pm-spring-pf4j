...
@Slf4j
public class PmPluginWebFluxRequestMappingHandlerMapping extends RequestMappingHandlerMapping {

    private final MultiValueMap<String, RequestMappingInfo> pluginRequestMappingInfo = new LinkedMultiValueMap<>();
    ...
    public Set<Object> registerControllers(String pluginId, ApplicationContext pluginAppContext) {
        ...
        try {
            ...
            log.info("Found {} WebFlux controllers in plugin {}: {}", controllers.size(), pluginId, controllerNames);
            ...
            log.info("Successfully registered {} WebFlux controllers for plugin: {} (took {} ms)", controllers.size(),
                    pluginId, duration);
            return controllers;
        } catch (Exception e) {
            long duration = System.currentTimeMillis() - startTime;
            log.error("Failed to register WebFlux controllers for plugin: {} (took {} ms)", pluginId, duration, e);
            throw e;
        }
    }

    public void registerHandlerMethods(String pluginId, Object handler) {
        Class<?> handlerType = (handler instanceof String beanName ? obtainApplicationContext().getType(beanName)
                : handler.getClass());
        ...
        if (handlerType != null) {
            ...
            Map<Method, RequestMappingInfo> methods = MethodIntrospector.selectMethods(userType,
                    (MethodIntrospector.MetadataLookup<RequestMappingInfo>) method -> super.getMappingForMethod(method,
                            handlerType));
            ...
            methods.forEach((method, mapping) -> {
                ...
                if (anonymousPathRegistrar != null) {
                    ...
                    if (methodAnno != null || classAnno != null) {
                        String reason = methodAnno != null ? methodAnno.reason() : classAnno.reason();
                        if (reason.isBlank()) {
                            log.warn(
                                    "[Plugin: {}] @AllowAnonymous on {}.{}() has blank reason — "
                                            + "registration as anonymous is blocked.",
                                    pluginId, handlerType.getName(), method.getName());
                        } else {
                            registerAnonymousPaths(pluginId, handlerType.getName(), method.getName(), mapping, reason);
                        }
                    }
                }
            });
    ...
    private void registerAnonymousPaths(String pluginId, String controllerClass, String methodName,
            RequestMappingInfo mapping, String reason) {
        if (mapping.getPatternsCondition() == null) {
            return;
        }
        Set<PathPattern> pathPatterns = mapping.getPatternsCondition().getPatterns();
        if (pathPatterns == null || pathPatterns.isEmpty()) {
            return;
        }
        List<String> patterns = pathPatterns.stream().map(pp -> pp.getPatternString()).toList();
    ...
    private Set<Object> getControllerBeans(ApplicationContext applicationContext) {
        ...
        if (log.isTraceEnabled()) {
            List<String> names = beans.stream().map(b -> b.getClass().getSimpleName()).collect(Collectors.toList());
    ...
    private void collectPluginPaths(String pluginId, RequestMappingInfo mapping) {
        ...
        if (patterns.isEmpty())
            return;
        Set<RequestMethod> methods = mapping.getMethodsCondition().getMethods();
        Map<String, Set<String>> pluginMap = pluginPaths.computeIfAbsent(pluginId, k -> new LinkedHashMap<>());
        if (methods.isEmpty()) {
